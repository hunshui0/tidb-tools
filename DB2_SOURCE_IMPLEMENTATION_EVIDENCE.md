...
The user also ran the current single-column keyset path with `chunk-size = 2`:
108 consecutive chunks were generated and processed, both endpoints used the
same stable range key, and one real differing field was localized. This is
evidence for that specific single-column multi-chunk scenario only; it does
