...
本轮新增的列差异修复已完成源码实现：DB2Source 对已存在对象保留同名列映射，
收集目标端独有列和 DB2 端独有列，标记结构比较并关闭 keyset/checksum-only 数据
路径。`StructEqual` 负责报告结构失败，分块迭代器为该表发送空 chunk 后继续后续表；
直接误调用 `GetRowsIterator` 会返回带表名和列名的明确错误，不会生成 `""` 标识符
SQL。`ignore-columns` 在映射前应用。该段的 sqlmock 测试属于离线证据，不等同于
Windows 原生构建或真实 DB2→TiDB 验收。

用户已在 Windows 上验证旧单分块版本的 Db2 CLI 真实连接、结构比较、GBK
中文转换和真实差异字段定位；随后又验证了当前单列 keyset 路径（`chunk-size = 2`，
